...
uniform int u_palette;     // PROTOTYPE colourways: which palette() branch to draw

out vec4 outColor;

// PROTOTYPE colourways. Six palettes on one uniform so the same view can be
// flipped between them in the browser. 0 is the original pen mapping.
// Every palette takes the smooth escape count t and cycles on a fixed period,
// like the original, so only the hues differ between variants.
...
export class Renderer {
  constructor(canvas) {
    ...
    this.lost = false;
    this.palette = 0; // PROTOTYPE colourways: index into palette()
  ...
  begin(view, fresh = true) {
    ...
    gl.uniform1i(prog.u.u_palette, this.palette | 0); // PROTOTYPE colourways
